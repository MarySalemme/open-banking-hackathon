export const setToken = code => async dispatch => {
  const res = await fetch(`http://localhost:3001/token?code=${code}`, {
    method: 'POST',
  });
  if (res.ok) {
...
export const getTransactions = (token, accountId) => async (
  dispatch,
  getState,
) => {
  // const token = getState().auth.access_token;
  const res = await fetch(`http://localhost:3001/transactions/${accountId}`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  if (res.ok) {
...
export const reviewTransactions = transactions => {
  ...
  let spendsMoreThanEarns =
    numMonthsSpendingMore / (numMonthsSpendingMore + numMonthsSpendingLess) >=
    0.3;
