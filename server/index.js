...
app.post('/token', function(req, tokenRes) {
  ...
  fetch('https://auth.truelayer.com/connect/token', {
    method: 'POST',
    body: form
  })
    .then(function(res) {
...
app.get('/accounts', function(req, accountsRes) {
  fetch('https://api.truelayer.com/data/v1/accounts', {
    method: 'GET',
...
app.get('/transactions/:account_id', function(req, transactionsRes) {
  fetch(
    ...
    }/transactions?from=2017-01-01&to=2018-01-01`,
    { method: 'GET', headers: { Authorization: req.header('Authorization') } }
  )
    .then(function(res) {
      if (res.ok) {
        return res.json();
